...
DEFAULT_PROMPT = (
    ...
    "There are exactly two line weights, never a gradual taper between them: a BOLD weight for "
    "major structural lines (silhouette/jaw outline, hair outline, glasses frame), and a THINNER "
    "accent weight (roughly half the bold weight) for small interior detail marks (nose, mouth, "
    "smile-crease lines, chin crease, ear fold). Within a single stroke the thickness never "
    "tapers, narrows, or comes to a point at either end -- every stroke holds its own weight "
    "(bold or thin) constantly along its whole length, with a rounded, not pointed, cap. Never a "
    "thin, delicate, or light line where the BOLD weight belongs, and never a heavy line where the "
    "accent weight belongs -- the contrast between the two weights matters and should be obvious. "
    "Match the reference avatars' actual line weight exactly: it should look hand-drawn and "
    "confident, not machine-clean. Fully anti-aliased with no wobble or pixel jaggedness. "
    "Keep the head a natural, moderate size relative to the shoulders -- not an oversized/cartoon "
    "head. "
    "CRITICAL: skin and face are bare white paper -- absolutely no fill, tint, tone, or shading of "
    "any kind on the face or neck, not even a light flat grey. Only ink linework sits on the white "
    "skin, exactly like the reference avatars. This is the single most important rule: if you find "
    "yourself filling the face with any color or tone, stop and leave it white instead. "
    ...
    "Eyes are small but have real internal structure, at accent weight: a short curved hook/lid "
    "stroke (the upper eyelid line) plus a separate small solid pupil dot sitting just beneath or "
    "inside its curve -- two distinct marks per eye, not one undifferentiated blob and not a full "
    "enclosed outline either. Base the exact eye shape/angle on this specific person's actual eyes "
    "in the photo rather than a generic template -- a little natural asymmetry between the two "
    "eyes is correct and expected, do not idealize them into a perfectly symmetric matched pair. "
    "Eyebrows are a bold curved stroke -- if the subject wears glasses, the eyebrow should sit "
    "close enough above the frame to almost touch or lightly overlap its top rim, reading as one "
    "continuous browline-into-glasses shape rather than two separate marks. Match this person's "
    "actual eyebrow arch/angle from the photo; slight asymmetry is correct, do not force a "
    "perfectly uniform curve on both sides. "
    "Nose, at accent weight: a small two-part construction -- a brief nostril hint near the tip "
    "(a shallow curved mark, not a large hooked shape) plus a short philtrum tick beneath it, "
    "separated from the nostril mark by a small visible gap so they read as two separate small "
    "marks, never touching or merging into one continuous line. Keep both marks genuinely small "
    "and light -- this is a hint of a nose, not a detailed rendering. Absolutely no line of any "
    "kind runs upward from the nose toward the eyes or eyebrows; there must be no visual "
    "suggestion of a nose bridge whatsoever. "
    "Mouth is a closed smile with real structure: one upper curve plus a shorter lower-lip line "
    "beneath it -- enough to read as an actual smile, not flattened down to a blank line. At "
    "accent weight, add one short curved smile-crease line beside each corner of the mouth "
    "(a nasolabial fold) if the photo shows the subject smiling, plus a single short accent-weight "
    "crease mark directly below the lower lip (a chin crease) separating the chin from the lip. "
    "These few accent marks are the ONLY extra detail permitted anywhere in the face -- do not add "
    "any other wrinkles, texture, or shading. Also add one short accent-weight fold line inside "
    "each visible ear, echoing its natural cartilage curve. "
    "Everywhere else in the drawing stays as sparse as possible: use the minimum number of "
    "strokes for every other feature, and when in doubt, omit a line rather than add one. The "
    "handful of accent marks just described (eyes, nose, mouth creases, chin, ears) are the "
    "exception, not the rule -- they carry this specific person's likeness and character, while "
    "everything else (forehead, cheeks, neck) stays completely bare. "
...
QA_PROMPT = (
    ...
    "1. The drawing uses exactly two line weights (a bold weight for the silhouette/jaw/hair/"
    "glasses, a visibly thinner accent weight for small interior details) -- not one single "
    "weight throughout, and not thin/hairline/generic-flat-vector-icon style overall.\n"
    ...
    "6. The nose is only a small nostril hint plus a separated philtrum tick beneath it -- no line "
    "runs upward from it that could read as a nose bridge, and it is not reduced to nothing at "
    "all.\n"
    "7. Each eye has two distinct marks (an eyelid/hook stroke plus a separate pupil dot) -- not "
    "one undifferentiated solid blob, and not a fully enclosed outline shape with a dot centered "
    "inside it.\n"
)
