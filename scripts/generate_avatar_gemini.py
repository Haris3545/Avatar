...
DEFAULT_PROMPT = (
    ...
    "Eyes are small: a single solid almond/leaf-shaped eye outline (one small curved pointed "
    "shape, like a content or slightly sleepy eye), with a small solid black pupil dot sitting "
    "inside it near the center -- not a bare dot with a separate tick above it, and never a large "
    "or realistic eye shape. "
    ...
    "separate marks. "
    "Nose is a small but clearly visible two-nostril 'gull-wing' mark: two shallow hook shapes "
    "meeting at a center dip, wide enough that the hooks at each end are actually visible -- not "
    "cropped down to a short flat/straight line, which reads as a nose bridge instead of a nose. "
    "Below it, separated by a visible small gap (not touching the nose mark), a short vertical "
    "philtrum tick leads down toward the upper lip. The nose mark and the philtrum tick must read "
    "as two separate small marks, never as one continuous vertical line. Do NOT draw a nose bridge "
    "line running up from the nostril mark toward the eyes/eyebrows -- the nose is ONLY the small "
    "gull-wing mark near the tip plus the philtrum tick beneath it, nothing above that connects it "
    "to the eyes. "
