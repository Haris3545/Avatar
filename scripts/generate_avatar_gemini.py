...
DEFAULT_PROMPT = (
    ...
    "use the minimum number of strokes possible for every feature. Every line is a bold, thick, "
    "confident stroke of one consistent heavy weight throughout, like a chunky marker or brush pen "
    "-- never a thin, delicate, or light line anywhere, and never the kind of uniform hairline-thin "
    "outline typical of generic flat vector icon art. Match the reference avatars' actual line "
    "weight exactly: it should look hand-drawn and bold, not machine-clean and thin. Fully "
    "anti-aliased with no wobble or pixel jaggedness. "
    ...
    "as two separate small marks, never as one continuous vertical line. "
    "Mouth is a closed smile with real structure: one upper curve plus a shorter lower-lip line "
    "beneath it -- enough to read as an actual smile, not flattened down to a blank line. Below "
    "the mouth, a short curved chin crease mark (separating the chin's own volume from the lip) "
    "-- do not leave the chin as bare unmarked skin down to the jaw outline. "
    ...
    "patch anywhere on the clothing, just one flat grey shape, a single clean collar shape (not a "
    "doubled or duplicated collar line), and one or two small button shapes drawn as hollow ring "
    "outlines (a small circle outline, not a filled dot) down the front. "
