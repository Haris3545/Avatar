...
DEFAULT_PROMPT = (
    ...
    "Hair is rendered as a flat mid-grey shape (not solid black) covered in dense directional "
    "linework -- many individual thin strand/wave strokes describing the actual hair shape and "
    "flow, like hand-inked hatching, plus a jagged/tufted hairline. This is much more linework "
    "than just a silhouette with a couple of accent marks: the hair should read as drawn stroke "
    "by stroke, not filled in. "
    "Eyes are a small solid pupil dot plus one thin curved upper-eyelid arc -- no other eye detail. "
    "Eyebrows are a thick, flat, solid black shape -- almost a small filled polygon, not a thin "
    "stroke -- clearly separated from the eyes and from glasses if worn. "
    "Nose is the smallest possible mark, often barely visible -- a tiny curved tick at most, never "
    "a full nostril outline. "
    "Mouth is a simple smile, either closed (one upper curve, one short lower lip line) or open "
    "with a visible gap between the two curves -- minimal detail either way. "
    "Glasses (if the subject wears them) are a bold, thick, fully continuous outlined frame with no "
    "gaps or breaks anywhere in the line, plus a couple of small diagonal reflection/sparkle tick "
    "marks near one upper lens corner. "
    "If the photo shows light stubble/a 5-o'clock shadow (not a full beard), draw it as a small "
    "halftone dot-pattern patch confined tightly under the chin only -- never spreading across the "
    "cheeks or upper lip. Only draw an actual full beard/moustache shape if the subject clearly has "
    "one covering the cheeks and/or upper lip in the photo. "
    "Clothing is a flat charcoal/near-black silhouette (darker than the hair's mid-grey, not a "
    "lighter grey), with a second, slightly different flat dark patch for shading on one side (no "
    "gradient), plus a couple of simple interior lines for a collar, seam, or button if the photo "
    "shows one. "
    "Background is solid flat white. "
    "No photorealistic shading, no gradients anywhere -- every tone is a flat, evenly-colored shape "
    "or a deliberate dot-pattern used only as described above, never randomly scattered noise. "
