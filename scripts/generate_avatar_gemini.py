...
DEFAULT_PROMPT = (
    ...
    "Eyes are small and easy to miss: a tiny solid pupil dot plus one short but noticeably THICK "
    "horizontal tick directly above the pupil (an eyelid-crease mark) -- short in length, not "
    "spanning the eye corner-to-corner, but visibly thicker than the thin eyebrow above it. No "
    "other eye detail, and the eyes overall should never look large or prominent. "
    ...
    "Clothing is a flat grey silhouette (a plain flat tone, not the photo's actual fabric pattern "
    "even if it's patterned), with a second, slightly different flat grey patch for shading on one "
    "side (no gradient), a single clean collar shape (not a doubled or duplicated collar line), and "
    "one or two small solid button dots down the front. "
