...
DEFAULT_PROMPT = (
    ...
    "use the minimum number of strokes possible for every feature, and when in doubt, omit a line "
    "rather than add one; a sparse drawing with only the essential marks is correct, not "
    "under-detailed. Every single line has the exact same uniform thickness along its ENTIRE "
    "length, like a rounded-tip marker held at a constant pressure -- it must NOT taper, narrow, "
    "or thin out at either end of a stroke. A stroke starts at full thickness, stays at that exact "
    "thickness the whole way, and ends at full thickness, with a rounded (not pointed) cap. This "
    "applies to every single line in the image without exception. Never a thin, delicate, or light "
    "line anywhere, and never the kind of uniform hairline-thin outline typical of generic flat "
    "vector icon art. Match the reference avatars' actual line weight exactly: it should look "
    "hand-drawn and bold, not machine-clean and thin. Fully anti-aliased with no wobble or pixel "
    "jaggedness. "
    ...
    "Eyes are small and extremely simple: a SINGLE SOLID FILLED black shape per eye, like a "
    "rounded comma or hook -- NOT an outlined eye shape with a separate pupil dot drawn inside "
    "it. There is only one mark per eye total, not two. It should read as a small, thick, solid "
    "ink blob with a gently curved hook shape, uniformly thick throughout its own length (not "
    "tapering to a point), never a large or realistic eye shape, and never an open outline. "
    "Eyebrows are a thick, bold curved stroke (not a thin hairline) -- if the subject wears "
    "glasses, the eyebrow should sit close enough above the frame to almost touch or lightly "
    "overlap its top rim, reading as one continuous browline-into-glasses shape rather than two "
    "separate marks. "
    "Nose is reduced to the absolute minimum: ONE single short curved line representing just the "
    "bottom/tip of the nose, nothing else. No separate nostril hooks, no philtrum tick, no second "
    "mark of any kind near the nose -- one line only. Absolutely no line of any kind runs upward "
    "from it toward the eyes or eyebrows; there must be no visual suggestion of a nose bridge "
    "whatsoever. If in doubt, make the nose mark even smaller/simpler rather than adding detail. "
    "Mouth is a closed smile with real structure: one upper curve plus a shorter lower-lip line "
    "beneath it -- enough to read as an actual smile, not flattened down to a blank line. "
    ...
    "Crop the final image tightly: the head and shoulders should fill most of the frame, with the "
    "bottom edge cutting off just below the shoulder line -- not showing the upper arms or torso, "
    "and not leaving empty space above the head either. Match the framing of the attached "
...
QA_PROMPT = (
    ...
    "5. Every line holds the exact same thickness along its entire length with rounded caps -- "
    "none of them taper, narrow, or come to a point at either end.\n"
    "6. The nose is drawn as a single simple curved line for the tip only -- no separate nostril "
    "marks, no philtrum mark, and no line running upward from it that could read as a nose "
    "bridge.\n"
    "7. Each eye is one single solid filled black shape (a rounded comma/hook) -- not an outlined "
    "eye shape with a separate pupil dot drawn inside it.\n"
)
